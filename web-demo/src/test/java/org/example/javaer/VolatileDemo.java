...
public class VolatileDemo {
    ...
    public void testVolatile() throws InterruptedException {
        Thread thread1 = new Thread(new Thread11());
        Thread thread2 = new Thread(new Thread11());
        thread1.start();
        thread2.start();
        thread1.join();
        thread2.join();
...
class Thread11 implements Runnable {
    private static int tickets = 0;  //多线程共享的变量
    ...
    public void run() {
        for(int i = 0; i < 50; i++) {
            tickets++;
            System.out.println(Thread.currentThread() + " count:" + tickets);
            Thread.sleep(100);
        }
    }
}
